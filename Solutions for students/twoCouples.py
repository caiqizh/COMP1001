# WEWEE, WEEEW, WWEWE, EEWEW, EWEWW, EWWWE are not part of the solution.
# The first three cannot be reached by EEEEE, while the latter three cannot be reached by WWWWW.
...
def solver():
    ...
    src = "EEEEE"                               # The beginning state where all are on the east side.
    des = "WWWWW"                               # The terminating state when the problem is solved
...
def genStates():
    ...
    states = ["".join((i,j,k,l,m)) for i in ["E","W"] for j in ["E","W"] for k in ["E","W"] for l in ["E","W"] for m in ["E","W"]]
    return tuple(states)
...
def isAStateLegal(state):
    ...
    if ((state[1] != state[2] and state[2] == state[3]) or \
        (state[3] != state[4] and state[4] == state[1]) or \
        # The boat is on the side where there is no one.
        (state[0] != state[1] and state[1] == state[2] == state[3] == state[4])):
        return False
    else:
        return True
